...
{
    [TestClass]
    public class GetPagedAllBannersByFilterAsync_Should : BannerServicesMock
    {
        [TestMethod]
        public async Task ReturnsAllBasnners()
        {
            ...
            using (var assertContext = new PayCloudDbContext(options))
            {
                ...
                //Act
                var testResult = await sut.GetPagedAllBannersByFilterAsync(0, 2, "");
        ...
        public async Task ReturnsEmptyList()
        {
            ...
            using (var assertContext = new PayCloudDbContext(options))
            {
                ...
                //Act
                var testResult = await sut.GetPagedAllBannersByFilterAsync(0, 3, "h");
